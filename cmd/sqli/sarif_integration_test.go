...
func TestSARIFOutputEndToEnd(t *testing.T) {
	...
	out := filepath.Join(t.TempDir(), "sqli.sarif")

	cmd := exec.Command("go", "run", "./cmd/sqli", "-sarif-output", out, "./sql/injection/testdata/src/y")
	cmd.Dir = repoRoot
	cmd.Env = append(os.Environ(), "GOCACHE="+filepath.Join(t.TempDir(), "gocache"))
	if output, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("go run sqli -sarif-output: %v\n%s", err, output)
	}
	...
	if got := doc.Runs[0].Tool.Driver.Name; got != "sqli" {
	...
	if rule.ID != "sqli/potential-sql-injection" {
	...
	if len(rule.Properties.Tags) < 2 || rule.Properties.Tags[0] != "security" || rule.Properties.Tags[1] != "external/cwe/cwe-89" {
